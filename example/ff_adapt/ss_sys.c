...
int ss_socket_m_event(int ss_fd)
{
    ...
    else if ( p_socket->status == SS_CLIENT )
    {
        ...
        if ( ss_buff_size(p_socket->pwrite) < (SS_BUFF_MAX_LEN - 1)  )
        {
            event |= EPOLLOUT;
        }
    }
...
void ss_buff_accept( struct ss_socket_m * p_socket )
{
    ...
    for (;;)
    {
        ...
        client_fd = ff_accept(p_socket->sockfd, &p_accept->addr, &p_accept->addrlen);
        if ( client_fd < 0 )
        {
            free(p_accept);
            break;
        ...
        p_accept->fd    = ( p_socket->idx + (p_socket->ref << 16) );
...
void ss_buff_connect(struct ss_socket_m * p_socket, int events )
{
    ...
    if ( events & EPOLLIN )
    {
        ...
        remain = ss_buff_size(p_socket->pread);
    ...
    if ( events & EPOLLOUT )
    {
        ...
        remain = ss_buff_read(p_socket->pwrite, stbuf, cnt );
        for ( ; remain > 0 ; )
        {
